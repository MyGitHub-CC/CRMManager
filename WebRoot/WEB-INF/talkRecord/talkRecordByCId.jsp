...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray">
		...
		当前状态：<%=customer.getStatus2().getStatus2() %>
		</div>
	</div>
...
<div id="pageGro" class="cb">
	<div class="pageBegin">
		...
		href="talkRecord?ye=1&customer.id=<%=cId%>"> 
			首页
		</a>
	</div>
	<div class="pageUp">
		<a class="customer-page-pre" 
		href="talkRecord?ye=<%=ye-1%>&customer.id=<%=cId%>"> 
			上一页
		</a>
	</div>
    <div class="pageList">
       <ul class="pagination">
			<%
			 ...
			 <li <%if (ye == i) {%>class="on" style="color:#fff;"<%} %> >
				 <a href="talkRecord?ye=<%=i%>&customer.id=<%=cId%>" 
    ...
    <div class="pageDown">
	    <a href="talkRecord?ye=<%=ye+1 %>&customer.id=<%=cId%>" > 
	    	下一页
	    </a>
    </div>
    <div class="pageEnd">
	    <a href="talkRecord?ye=<%=maxPage %>&customer.id=<%=cId%>" > 
	 		  尾页
	    </a>
    </div>
</div>
			...
			<div class="modal-body" id="modal-body-table">
				<article class="page-container">
				 <form method="post" class="form form-horizontal" >
					...
					<input type="hidden" name="customer.id" class="customerId" value="<%=customer.getId() %>" />
					<div class="row cl">
	...
	$("#submit-customer").click(function(){
		...
		$.ajax({
			...
			success: function(data){
				alert(data);
				if(data == "保存成功"){
					location.href="talkRecordByCId?customer.id=<%=cId %>";
				}
			}
		});
	});
