...
<%
	List<Statistics> statis = (List<Statistics>) request.getAttribute("statis");
 %>
<body>
...
<div class="page-container">
	<div class="f-14 c-error">青岛大众4S店-销售统计分析图：。</div>
        ...
        series: [{
            ...
            data: [
            	...
            	for(int i = 0;i < statis.size(); i++){
            		Statistics sta = statis.get(i);
            		if(i != statis.size()-1){
            	%>
	                ['<%=sta.getCustomer().getStatus2().getStatus2()%>',   <%=sta.getPre()%>],
            	<%}else{%>
            		 {
                    name: '<%=sta.getCustomer().getStatus2().getStatus2()%>',
                    y:  <%=sta.getPre()%>,
                    sliced: true,
                    selected: true
               		 },
